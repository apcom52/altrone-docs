import {
  CONFIGS,
  ConfigurationOption,
} from "@/app/components/configuration/configurations";
...
const configCellRenderer = ({
  value,
  item,
}: DataTableCellProps<ConfigurationOption>) => {
  const path = String(value).split(".");

  const newLabel =
    item.availableSince === VERSION ? (
  ...
  if (path.length > 1) {
    return (
      <div className={s.ConfigCell}>
        ...
        <span className={s.LastLeaf}>{path.at(-1)}</span>
        {newLabel}
      </div>
    );
  }
  ...
  return (
    <div className={s.ConfigCell}>
      <span className={s.LastLeaf}>{path[0]}</span>
      {newLabel}{" "}
    </div>
  );
};
