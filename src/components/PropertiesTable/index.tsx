...
import {
  ...
  ComponentType,
} from "@/types/ComponentProperties";
...
const NameCell = ({ value, item }: DataTableCellProps<ComponentType>) => {
  const deprecatedLabel = item.deprecated ? (
    <div className={s.DeprecatedLabel}>deprecated</div>
  ) : null;
  const newLabel =
  ...
  return (
    <div className={s.PropNameColumn}>
      {String(value)}
      {item.required ? (
        <div className={s.Asterisk} title="Required property">
          *
        </div>
      ) : null}
      {newLabel}
...
const DefaultValueCell = ({ value }: DataTableCellProps<ComponentType>) => (
  <Text.Code>{String(value)}</Text.Code>
);

export const PropertiesTable = memo(
  ({ title = "Properties", role = "heading", data }: CheckAlsoProps) => {
    return (
      <Text.Section>
        {data.extends ? (
          <Text.Paragraph>
            This component has all properties from{" "}
            <Text.Code>{data.extends}</Text.Code>
          </Text.Paragraph>
        ) : null}

        <div className={s.PropertyTableWrapper}>
