...
export const PageNavigation = ({ page }: PageNavigationProps) => {
  ...
  return <div className={s.Container}>
    <div className={s.LeftColumn}>
      {prevPage ? <Link href={`/components/${prevPage.name}`} className={s.PageButton} {...rainbowProps}>
        <div className={s.ButtonTitle}>Previous</div>
        <div className={s.ButtonContent}>{prevPage.title}</div>
      </Link> : null}
    </div>
    <div className={s.RightColumn}>
      {nextPage ? <Link href={`/components/${nextPage.name}`} className={s.PageButton} {...rainbowProps}>
